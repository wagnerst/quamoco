 ...
 */
// TODO comment
public class AttributesSettingPageFactorHierarchyPropertyDescriptor extends ItemPropertyDescriptor {

	final static boolean isSettable = true;
	final static boolean multiLine = false;
	final static boolean sortChoices = true;
	
	public AttributesSettingPageFactorHierarchyPropertyDescriptor(AdapterFactory adapterFactory, ResourceLocator resourceLocator, String displayName,
	 ...
	 * that do not describe hierarchies of {@link Factor}s.
	 */
	@Override
	protected Collection<?> getComboBoxObjects(Object object) {
		...
		for (Iterator<?> iterator = result.iterator(); iterator.hasNext();) {
			...
			if (entry instanceof ElementHierarchy) {
				...
				if (!(hierarchy.getReference().getEType() == QmPackage.Literals.FACTOR)) {
					iterator.remove();
				}
			}
		}		
