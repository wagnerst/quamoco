...
public class WeightedSumMultiMeasureEvaluationInvalidRankingConstraint extends
		ElementModelConstraintBase {

	@Override
	protected IStatus validate(EObject target) {
		WeightedSumMultiMeasureEvaluation evaluation = (WeightedSumMultiMeasureEvaluation) target;
		int numberOfRankingNot0 = 0;
		for (Ranking r : evaluation.getRankings()) {
			if (r.getRank() != 0) {
				numberOfRankingNot0++;
			}
		}

		if (numberOfRankingNot0 == 0) {
