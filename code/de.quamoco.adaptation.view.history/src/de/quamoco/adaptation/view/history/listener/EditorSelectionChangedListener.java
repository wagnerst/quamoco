...
package de.quamoco.adaptation.view.history.listener;
...
public class EditorSelectionChangedListener implements ISelectionListener {
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		if (part instanceof QmEditor) {
			List<AdaptationHistoryItem> itemsToSelect = new LinkedList<AdaptationHistoryItem>();
					...
					if (object instanceof AnnotatedElement) {	// add all tasks to result
						for (AnnotationBase baseAnnotation : ((AnnotatedElement) object).getAdvancedAnnotations()) {
							if (baseAnnotation instanceof AdaptationHistoryItem) {
								itemsToSelect.add((AdaptationHistoryItem) baseAnnotation);
							}
						}
			...
			treeViewer.setSelection(new StructuredSelection(itemsToSelect), reveal);
		}
	}
